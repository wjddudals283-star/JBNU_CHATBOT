...
def test_제목이_별도_줄인_캡션(sep):
    """지금까지 들어온 표본은 전부 이 모양이었다 — 줄바꿈이 구분자였다."""
    assert templates.strip_leading_title(f"[제목]{sep}첫 줄", "제목") == "첫 줄"
...
def test_제목과_본문이_한_줄인_캡션(sep):
    """★ 한 줄짜리 캡션이 처음 들어온 자리다 (2026-08-14).

    그전 표본은 제목이 늘 별도 줄이라 **줄바꿈이 구분자 노릇**을 했다.
    실제 캡션이 '[제목] / 모집 기간: …' 으로 오자 '/' 가 그대로 남았다 —
    구분자 목록에 '-' 만 있었기 때문이다.
    어느 기호를 쓸지는 총학이 정하므로 넓게 잡는다.
    """
    got = templates.strip_leading_title(f"[제목]{sep}첫 줄", "제목")
    assert got == "첫 줄", repr(got)


def test_실제로_들어온_한_줄_캡션():
    """배포본에서 나온 그 모양 그대로."""
    body = ("[2026 학문체 댄스제 참가자 모집] / "
            "모집 기간: 2026.08.12.(수) ~ 08.25.(월)")
    got = templates.strip_leading_title(body, "2026 학문체 댄스제 참가자 모집")
    assert got.startswith("모집 기간:"), repr(got)
