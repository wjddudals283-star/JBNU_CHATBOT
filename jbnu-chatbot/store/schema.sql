...
-- ═══════════════════════════════════════════════════════════════
-- 어휘 사전 — 붙여 쓴 말을 쪼개기 위한 **관측된 낱말**
-- ═══════════════════════════════════════════════════════════════
-- ★ 왜 필요한가 (2026-08-15)
--   등록 발화가 전부 띄어쓴 형태라 '졸업 요건' 은 걸리고 '졸업요건' 은 폴백이다.
--   그리고 검색 토큰화가 공백으로 자르므로 붙여 쓰면 통째로 한 토큰이 된다.
--   46문항을 붙여 써 보니 **35/38 건에서 답이 달라졌고** 대부분 '못 찾았어요' 였다.
--
-- ★ 사전은 관측이다. 우리가 적는 목록이 아니다
--   코퍼스에 **공백으로 구분되어 실제로 나타나는** 낱말을 센다.
--   조사가 붙은 형태('성적이')도 들어오지만 DF 가 낮아 분해에서 진다 —
--   가설이 아니라 실측이다: '성적이의신청' → 성적|이의|신청 (최장일치는 성적이|의신청)
--
-- ★ 답변 경로는 조회만 한다. 만드는 건 크롤 단계다 (T7: p95 < 300ms)
CREATE TABLE IF NOT EXISTS term (
  term  TEXT PRIMARY KEY,
  df    INTEGER NOT NULL DEFAULT 0,      -- 이 낱말이 나온 문단 수
  built_at TEXT NOT NULL DEFAULT ''
);

CREATE INDEX IF NOT EXISTS idx_term_df ON term(df DESC);
