# 노트북 백필 세팅 — 생협 원천

작성일 2026-08-11 · **2026-08-14 개정: 사람이 기억하는 구조를 버렸다**

> ★ 이 문서의 '주 2회 손으로' 부분은 **폐기됐다.**
>   대표가 이 프로젝트에 처음 건 조건이 '자동 크롤로 사람이 관리하지 않아도
>   계속 업데이트' 였고, 주 2회 수동 백필은 그걸 정면으로 깬다.
>   실제로 8/13(목) 백필이 안 돌았다. 개강하면 총학이 제일 바쁠 때다.
>
>   → 지금은 **대표 PC 작업 스케줄러**가 하루 4회 자동으로 돈다.
>     '놓친 작업 실행' 옵션 덕에 PC 를 켜기만 하면 밀린 것이 저절로 돈다.
>     설정 절차: **docs/PC_SCHEDULER.md**
>
>   아래 내용은 원리와 수동 실행 방법으로 남긴다 (인수인계·문제 해결용).

---

## 왜 필요한가

생협 API 가 **Render(해외 IP)에서 403** 이다. 헤더 변형 7종과 일반 웹페이지까지
전부 막혔다(IP/지역 차단). 한국 IP 에서는 200 이다.

생협은 **1차 원천**이고, `date` 파라미터로 과거 조회가 되는 유일한 식단 원천이다.
학교 XHR 은 이번 주 고정이라 놓친 주를 영영 못 채운다. 그래서 우선순위를 안 바꾸고,
**한국 IP(노트북)에서 백필**하는 우회로 간다.

> 답변 품질은 이미 안 깨진다. `merge()` 가 "1차에 없고 2차에만 있는 칸은 2차 채택"
> 으로 처리하므로 학교 XHR 만으로 A분기가 나온다. 백필로 되찾는 건
> **교차검증 신호**와 **과거 데이터**다.

---

## 주기 — 주 2회 (월·목)

크롤 주기는 **신선도 임계 ÷ 2** 이하여야 한다. 같으면 여유가 0이라
한 번만 걸러도 즉시 stale 이 된다.

| | 값 |
|---|---|
| 생협 신선도 임계 | 192h (8일) |
| 허용 최대 간격 | 96h (4일) |
| 주 1회 | 168h → **위반** |
| **주 2회 (월·목)** | 96h → OK |

`config/sources.yaml` 의 `effective_cadence_hours: 96` 이 이 값이고,
`tests/test_schedule.py::test_모든_원천이_주기_규칙을_지킨다` 가 지킨다.

---

## 흐름 — 노트북은 네트워크 위치만 빌려준다

```
노트북(한국 IP) → 생협 API 200 → 원문 바이트
        ↓ POST /admin/ingest  (X-Skill-Token)
서버 → **다시 파싱** → 검증 게이트 → 정렬 앵커 → 교차검증 → DB
        ↓
학생 답변에 즉시 반영
```

★ **파싱된 레코드가 아니라 원문 바이트를 보낸다.**
서버가 파서·게이트를 처음부터 다시 통과시킨다. 밖에서 들어온 데이터를
그대로 믿지 않는다. 노트북에서 이미 검증했더라도 서버가 한 번 더 본다.

★ **출처 메타는 노트북 것을 유지한다.** `observed_at` 은 노트북이 받아온 시각이다.
서버가 받은 시각으로 바꾸면 신선도가 실제보다 좋아 보인다.

★ 해시는 서버가 다시 계산한다. 클라이언트가 준 값을 받지도 않는다.

---

## 등록 절차 (Windows 작업 스케줄러)

### 1. 토큰 준비

Render Dashboard → 서비스 → **Environment** → `SKILL_TOKEN` 옆 눈 아이콘 → 값 복사.

> 이 값을 채팅·메신저에 붙여넣지 않는다. 붙는 순간 시크릿이 아니게 된다.

### 2. 배치 파일 만들기

`jbnu-chatbot` 폴더에 `backfill.bat` 을 만든다. 경로와 토큰은 본인 것으로.

```bat
@echo off
cd /d "C:\Users\정영민\Desktop\전북대총학_카톡챗봇\jbnu-chatbot"
set SKILL_TOKEN=여기에_토큰
set JBNU_SERVER=https://jbnu-chatbot.onrender.com
python -m crawler.push --source coop_week_menu >> data\backfill.log 2>&1
```

> `>>` 로 로그를 남긴다. 조용히 실패하면 몇 주 뒤에나 알게 된다.
>
> ⚠️ 이 `.bat` 에는 토큰이 들어간다. **저장소에 커밋하지 않는다**
> (`.gitignore` 가 `*.bat` 을 막지는 않으므로 직접 확인할 것).
> 파일을 개인 폴더에 두고 공유하지 않는다.

### 3. 작업 스케줄러 등록

1. 시작 → **작업 스케줄러** 실행
2. 오른쪽 **기본 작업 만들기**
3. 이름: `전북대챗봇 생협백필`
4. 트리거: **매주** → 시작 `오전 9:00` → **월요일, 목요일** 체크
5. 동작: **프로그램 시작** → 찾아보기 → 위에서 만든 `backfill.bat`
6. 마침 → **[속성] 대화 상자 열기** 체크 후 완료
7. 속성 → **조건** 탭
   - "컴퓨터의 AC 전원이 켜져 있는 경우에만..." **체크 해제** (노트북이면 중요)
8. 속성 → **설정** 탭
   - **"예약 시간을 놓친 경우 가능한 대로 빨리 작업 시작" 체크** ★
   - 노트북이 꺼져 있던 날을 따라잡는다. 이게 없으면 그 주는 통째로 빈다

### 4. 확인

작업 스케줄러에서 해당 작업을 우클릭 → **실행** 후 로그를 본다.

```powershell
Get-Content jbnu-chatbot\data\backfill.log -Tail 20
```

이렇게 나오면 정상이다.

```
서버: https://jbnu-chatbot.onrender.com
  [coop_week_menu] fetch 200  16,515B
    → 서버 success  식단 95건 / 격리 0건  OK

1/1 성공
```

| 증상 | 뜻 |
|---|---|
| `fetch 403` | **노트북도 차단됐다.** 캠퍼스 망에서 다시 시도 |
| `서버가 거부했다 401` | 토큰이 틀렸다. Render 에서 다시 복사 |
| `서버가 거부했다 400 ... 미래` | 노트북 시계가 틀어졌다 |
| `서버 parse_error` | 원천 구조가 바뀌었다. 파서 수정 필요 |
| `서버 unchanged` | 이미 같은 내용이 들어가 있다. 정상 |

---

## 과거 주 채우기

특정 주를 놓쳤으면 날짜를 지정한다.

```bash
python -m crawler.push --source coop_week_menu --date 2026-09-07
```

지난 몇 주를 한 번에 채우려면:

```bash
python -m crawler.push --source coop_week_menu --weeks-back 4
```

같은 주를 두 번 넣어도 `UNIQUE` 가 막으므로 중복이 안 쌓인다.

---

## 서버가 다시 검증한다 — 확인된 동작

노트북이 뭘 보내든 서버가 게이트를 다시 통과시킨다. 테스트로 고정돼 있다.

| 보낸 것 | 서버 결과 |
|---|---|
| 정상 JSON | `success` · DB 반영 |
| `status: fail` 응답 | `parse_error` · **기존 데이터 무변경** |
| 구분자 놓친 항목 | 그 항목만 `quarantine` |
| 403 본문 | `fetch_error` · 사유 기록 |
| 미래 시각 | `400` 거부 |
| 5MB 초과 | `413` 거부 |

---

## 로컬 DB 는 어떻게 되나

`python -m crawler.push` 는 **서버로만** 보낸다. 노트북 로컬 DB 에는 안 쌓인다.
로컬에도 남기고 싶으면 따로 돌린다.

```bash
python -m crawler.run --source coop_week_menu    # 로컬 DB
python -m crawler.push --source coop_week_menu   # 서버 DB
```

평소에는 `push` 만 하면 된다. 학생 답변에 반영되는 건 서버 DB 다.

---

## 차단이 풀리면

배포 서버가 매일 스모크를 돌린다. 풀리면 로그에 이렇게 뜬다.

```
[smoke] BLOCK-LIFTED source=coop_week_menu 차단이 풀렸다 (통과: [...]).
```

그때 `config/sources.yaml` 의 `known_blocked` 와 `effective_cadence_hours` 를 지우고
`stale_after_hours` 를 24 로 되돌린 뒤 커밋하면 1차 원천이 복구된다.
