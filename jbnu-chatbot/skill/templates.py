...
#   위 기준(첫 화면에 되묻기가 나오면 안 된다)은 **안 될 것을 거르는** 자다.
#   무엇을 **넣을지**는 안 정해 준다. 그래서 '자퇴 절차' 가 첫 화면에 있었다.
#   기준: **첫 화면은 학생이 실제로 많이 묻는 것이어야 한다.**
#   8/27 까지 폴백 101종에서 휴학·자퇴 계열은 상위에 없고,
#   후생관 학식 8건 · 총학생회 공지 4건은 있다.
#
# ★ 그리고 **두 벌이었다** — 이게 본질이다
#   오픈빌더 웰컴 블록에도 버튼이 붙어 있고 여기에도 있었는데 서로 달랐다.
#   '처음으로' 가 폴백 1위(27건)라 **코드 쪽 웰컴이 오히려 더 많이 보인다.**
#   많이 보이는 쪽이 카카오 첫 화면과 다르면
#   학생은 같은 봇을 두 번 처음 만난다. 오픈빌더에 맞춘다.
WELCOME_MENU = ["수강신청 언제야", "오늘 학식", "총학생회 공지"]
...
def _dept_example(hits) -> str:
    """예시로 쓸 학과 이름 — **그 주제 문서를 실제로 가진 학과**에서 고른다.

    ★ 지어내지 않는다. 후보에 있는 학과를 그대로 쓰면
      학생이 "아 저렇게 쓰는 거구나" 를 눈으로 확인할 수 있다.

    ★ 두 번 틀리고 **구체 학과를 포기했다** (2026-08-28)
      1차  items 의 title 에서 골랐다 — 그건 페이지 제목일 수 있다.
      2차  site_name 에서 골랐다 — 그래도 '영어영문학과 수강신청 정정' 이
           또 되물었다. **후보에 있다 ≠ 그 학과가 그 문서를 가졌다.**
           본문에서 스쳐서 후보에 오른 것일 수 있다.
      제안한 형태를 그대로 쳤는데 안 되면 **안내가 거짓말**이다.
      확인 안 된 것을 단정하지 않는다 — 이 프로젝트의 첫 원칙이다.
      그래서 구체 학과 대신 **자리표시자**를 쓴다. 지어낸 학과가 아니니
      틀릴 수가 없고, 학생은 어차피 자기 학과를 넣는다.

    ★ 후보에 학과가 하나도 없으면(본부·센터만) 빈 문자열 —
      그때는 안내 자체를 안 붙인다. 없는 학과를 지어내면 그게 답인 줄 안다.
    """
    for h in (hits or []):
        name = (getattr(h, "site_name", "") or "").strip()
        if name.endswith(("학과", "학부")):
            return "○○학과"      # 후보에 학과가 섞여 있다는 것만 쓴다
    return ""
    ...
    if result.outcome is Outcome.AMBIGUOUS:
        ...
        if missing:
            ...
            if len(items) < 2:
                ...
                #   되묻기 3턴 13건 **전부** 그랬다.
                #
                #   ★ 그래서 **붙여 쓴 형태**를 알려준다. 이제 그게 실제로 된다 —
                #     b313192(사이트 좁히기 고침) 뒤 '학과 + 주제' 정확도 95%.
                #     어순은 무관하지만(둘 다 95%) 다른 형식 안내와 같은 순서로 쓴다.
                #
                #   ★ 목록에 **없는 학과도 된다**고 말한다
                ...
                if dept_dependent:
                    tail = (f"학과마다 내용이 달라요.\n"
                            f"'{ex} {subject}'처럼 학과를 붙여서 물어봐 주세요.\n"
                            f"목록에 없는 학과도 괜찮아요.")
                elif ex:
                    # ★ 축이 하나 더 필요했다 (2026-08-28)
                    #   dept_dependent 는 '본부 문서가 후보에 있나' 로 정해진다.
                    #   '질병휴학' 은 본부 문서가 있어서 False 인데,
                    #   **대표가 겪은 화면이 정확히 이것**이었다 —
                    #   목록에 본부와 학과가 섞여 있고 자기 과가 없었다.
                    #
                    #   이건 세 번째 축이다: **후보에 학과가 섞여 있나.**
                    #   items 에서 바로 읽는 관측이고, 말하는 것도 다르다 —
                    #     dept_dependent  "학과마다 내용이 달라요"   (사실 주장)
                    #     이 자리          "이렇게 물어볼 수도 있어요" (사용법)
                    #   사실을 주장하지 않으니 틀려도 학생을 속이지 않는다.
                    tail = ("어느 쪽을 찾으시는지 눌러서 확인해 주세요.\n"
                            f"학과를 알면 '{ex} {subject}'처럼 붙여서 "
                            "물어보셔도 돼요. 목록에 없는 학과도 괜찮아요.")
                else:
                    tail = "어느 쪽을 찾으시는지 눌러서 확인해 주세요."
